...
class DidWebEndorser:
    ...
    async def did_registration(self, namespace, identifier, url=None):
        ...
        did_document = DidDocument(
            ...
            verificationMethod=[
                VerificationMethod(
                    id=f"{did}#key-01", controller=did, publicKeyMultibase=multikey
                )
            ],
        ).model_dump()
        # if url:
        ...
        endorsed_did_doc = await AskarWallet().add_proof(
            signed_did_doc, endorser_options
        )
    ...
    def request_did(self, namespace, identifier):
        ...
        try:
            ...
            raise HTTPException(
                status_code=r.status_code, detail="Couldn't request did."
            )

    def register_did(self, endorsed_did_doc):
        r = requests.post(f"{self.server}", json={"didDocument": endorsed_did_doc})
        try:
            return r.json()["didDocument"]
        except:
            raise HTTPException(
                status_code=r.status_code, detail="Couldn't register did."
            )
