...
with open('app/related_resources/credential_types/PetroleumAndNaturalGasTitle.json') as f:
    EXAMPLE_CREDENTIAL_TYPE = json.loads(f.read(), object_pairs_hook=OrderedDict)
    
class RegisterCredential(BaseModel):
    credentialRegistration: CredentialRegistration = Field(example=EXAMPLE_CREDENTIAL_TYPE)
...
EXAMPLE_ISSUANCE_OPTIONS = {
    'credentialType': 'BCPetroleumAndNaturalGasTitleCredential'
}
